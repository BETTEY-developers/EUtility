...
public class Union<UnT1, UnT2> : ICloneable, IVoidTestable
{
    ...
    public object Clone()
    {
        ...
        r = new(this._T1Value);
        r._T2Value = this._T2Value;
        r._hasValue = this._hasValue;
        return r;
    }
    ...
    public T? GetValue<T>()
    {
        if (typeof(T) == typeof(UnT1))
            ...
            return (T)Convert.ChangeType(GetValueT2(), typeof(T));
        else
            return default;
    }
    ...
    public bool IsVoid()
    {
        return GetValue<Void>() != default;
    }

    public override string ToString()
    {
        if (CheckT1(out var v1))
            return v1.ToString();
        else if (CheckT2(out var v2))
            return v2.ToString();
        return "empty union";
    }
}